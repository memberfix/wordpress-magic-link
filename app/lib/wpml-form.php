    ...
    class WPML_Form{
        ...
        public function option(array $arguments, string $select_id): void
        {
            if(!isset($this->inputs[$select_id])){
                wpml_dd("The select element doesn't exist");
            }
        ...
        private function add_element(array $arguments, string $element, $type = false): void
        {
            if(!$this->validation($arguments)){
                wpml_dd('The required fields were not added to the form');
            }
        ...
        private function render_input(array $input): void
        {
	        if(isset($input['group']) &&$input['group'] == 'start'){
		        echo "<div class='wpml-form-group'>";
	        }

            echo '<'. $input['element'] .' type="' . $input['type'] . '" id="'. $input['id'] . '"' . ((isset($input['name'])) ? 'name="' . $input['name'] . '"' : '') . ((isset($input['placeholder'])) ? $input['placeholder'] : '') . ((isset($input['value']) || isset($input['default'])) ?'value="' . $this->set_element_value($input) . '"' : '') . 'class="wpml-input ' . (( $input['class'] ?? '' )) . (( $input['type'] == 'submit' ? ' wpml-submit' : '')) . '"' . (($input['checked']) ? 'checked' : '') .'>';
        ...
        private function render_select(array $input): void
        {
            ...
            echo '<' . $input['element'] . ' name="' . $input['name'] . '" id="' . $input['id'] . 'class="wpml-input ' .((isset($input['class']) ? $input['class'] : '')) . '">';
        ...
        private function render_label(array $input): void
        {
            ...
            echo '<label id="'. $input['id'] . '" class="wpml-input wpml-label ' . ((isset($input['class']) ? $input['class'] : '')) . '" for="' . $input['for'] . '">';
            echo $input['label'];
            echo '</label>';
	    ...
	    private function render_heading(array $input): void
	    {
		    ...
		    echo '<'. $input['element'] .' id="'. $input['id'] . '" class="wpml-heading wpml-form-heading ' . ((isset($input['class']) ? $input['class'] : '')) . '">';
		    echo $input['content'];
		    echo '</'. $input['element'] .'>';
